...
class AntropomorphicEndEffectorMover(object):
    ...
    def __init__(self, wait_reach_goal=True):
        ...
        self.elbow_policies = ["plus-plus", "plus-minus", "minus-plus", "minus-minus"]
        ...
        rospy.loginfo("antropo init done")
    ...
    def ee_pose_commands_clb(self, msg):
        ...
        DH_parameters={"r1":r1,
            "r2":r2,
            "r3":r3}
        theta_2_config, theta_3_config = self.elbow_pol.split("-")
        theta_array, possible_solution = calculate_ik(
            Pee_x=self.Pee_x, Pee_y=self.Pee_y, Pee_z=self.Pee_z,
            DH_parameters=DH_parameters, 
            theta2_config = theta_2_config, theta3_config=theta_3_config)

        if not(possible_solution):
            for pol in self.elbow_policies:
                if pol != self.elbow_pol:
                    theta_2_config, theta_3_config = self.elbow_pol.split("-")
                    theta_array, possible_solution = calculate_ik(
                        Pee_x=self.Pee_x, Pee_y=self.Pee_y, Pee_z=self.Pee_z,
                        DH_parameters=DH_parameters, 
...
def main():
    ...
    planar_object = AntropomorphicEndEffectorMover()
    rospy.spin()
